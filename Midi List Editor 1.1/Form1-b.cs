...
namespace Midi_List_Editor_1._1
{
    ...
    {
        public string[] fileData = new string[23];  // information in Header Chunk
        int nbTr = new int();
 ...
 /*       List<List<int>> lst20 = new List<List<int>>();
        ...
        List<List<byte>> sysData = new List<List<byte>>(); // information in sysex commands
        ...
        private void bList_Click(object sender, EventArgs e)
        {
            ...
            int cntlst3 = lst3.Count();
            if (trackNr > cntlst3)
            {
                textBoxList.AppendText("Track do not exist");
                return;
            }
            int nextTime = 0;
            if (trackNr == 0)
            ...
            if (trackNr > 0)
            {
                ...
                //while (i < 100)
                {
                    ...
                    switch (state)
                    {
                        case 240:
                            //sysex();
                            str = str + "Sysex; length: " + lst3[nr][i][2].ToString();
                            //str = str + "                      ";
                            break;
                        case 255:
        ...
        {
            Form2 myForm;
            myForm = new Form2(lst3, strTr, fileData, sysData);
            myForm.Show();
        }
            ...
            if (result == DialogResult.OK) // Test result.
            {
                ...
                while (trNum < nTr + 1)
                {
                    ...
                    while (arrpos < iLoop)
                    {
                        ...
                        {
                            case 240:
                                arrpos = sysex(vect, array, arrpos);
                                //MessageBox.Show("sysex; pos= " + Convert.ToString(arrpos));
                                break;
                            case 255:
                                arrpos = metaEv(vect, array, arrpos,Time, ref Bar);
                                //MessageBox.Show("meta event; pos= " + Convert.ToString(arrpos));
                                break;
                            default:
        ...
        private void bSave_Click(object sender, EventArgs e)
        {
            ...
            while (i < nbTr)
            {
                ...
                while (nb<trLen)
                {
                    ...
                    {
                        case 240:
                            byte sysexLength = (byte)lst3[i][nb][2];
                            //outarr.Add(sysexLength);
                            byte ib = 0;
                            while (ib < sysexLength + 1)
                            {
                                outarr.Add(sysData[lst3[i][nb][3]][ib]);
                                ib++;
                            }
                            break;
                        case 255:
        ...
        public int sysex(int[] vect, byte[] arr, int nT)
        {
           // MessageBox.Show("Sysex");
            ...
            while (i < len)
            {
                sysVect.Add(arr[nT + i]);
                i++;
            }
            vect[3] = sysData.Count;
            sysData.Add(sysVect);
            return nT + len;
        }
    ...
    public class midlist
    {
        ...
        public void toTwo(int[] vect, List<List<int>> lst2)
        {
            List<int> lst1 = new List<int>();
            int[] arr = new int[4];
            arr = vect;
        ...
        public void toThree(List<List<int>> lst2, List<List<List<int>>> lst3)
        {
            List<List<int>> listD = lst2.ToList();
            //listD = lst2;
            lst3.Add(listD);
        }

        public void listClone(List<List<List<int>>> lst3a, List<List<List<int>>> lst3b)
        {
            int[] vect = new int[4];
            List<List<int>> lst2 = new List<List<int>>();
            int i = 0;
            ...
            List<List<List<int>>> listD=lst3a.ToList();
